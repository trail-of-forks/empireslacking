...
class SlackData extends EventEmitter {
  ...
  async init() {
    ...
    this.client.team.info().then((res) => {
      const { team } = res;
      if (!team) {
  ...
  async fetchUserCount() {
    ...
    const getUsersWithCursor = async (cursor) => {
      let response = null;
      ...
      do {
        try {
          ...
          this.emit('error', `Error status: ${error.response.status}`);
    ...
    do {
      cursor = await limiter.schedule(() => getUsersWithCursor(cursor));
    } while (cursor);
